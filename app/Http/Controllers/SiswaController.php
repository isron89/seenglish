...
class SiswaController extends Controller
{
  ...
  {
    $siswas = User::where('status', 'S')->latest()->get();
    if (auth()->user()->status == 'G') {
      ...
      return Datatables::of($siswas)
        ...
        ->rawColumns(['status_validasi', 'action'])
        ->make(true);
    }
  }
  ...
  public function validasi(Request $request)
  {
    ...
    $token_ujian = md5($str);
    $query->status_validasi = $request->validasi;
    ...
    $query_keuangan->save();
    $details = [
      'title' => 'Validasi Pembayaran Ujian',
      'body' => 'Pembayaran anda telah divalidasi, berikut token adalah token yang digunakan untuk ujian',
      'text' => ''. $token_ujian .''
    ];
    Mail::to($query->email)->send(new \App\Mail\SendTokenMail($details));
    return redirect('master/siswa/detail/' . $request->id)->withSuccess('Data Tervalidasi');
  }

  public function daftarUjian()
  {
    $user = User::where('id', auth()->user()->id)->first();
    $kelas = Kelas::where('id', '!=', auth()->user()->id_kelas)->get();
    return view('halaman-siswa.daftar_ujian', compact('user', 'kelas'));
  }
  ...
  // {
  //   $string = auth()->user()->email .' - '.auth()->user()->id_kelas;
  //   if (md5($string) == $request->token) { 
  ...
  //     } else {
  //       return redirect()->back()->withErrors('Token salah')->withInput();
  //     }
  //   } else { 
  //     return redirect()->back()->withErrors('Token tidak terdaftar')->withInput();
  //   }
  // }
